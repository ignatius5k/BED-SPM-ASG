    ...
    async function loadNavBadges() {
      ...
      try {
        const response = await fetch(
          `http://localhost:3000/vendorBadges/${encodeURIComponent(vendorId)}`
        );
        if (!response.ok) return;

        const counts = await response.json();
        const badgeValues = [
          ['notification-badge', counts.notifications],
          ['mobile-notification-badge', counts.notifications],
          ['complaint-badge', counts.complaints],
          ['mobile-complaint-badge', counts.complaints]
        ];

        badgeValues.forEach(([id, count]) => {
          const badge = document.getElementById(id);
          if (!badge || !Number(count)) return;
          badge.textContent = count;
          badge.classList.add('show');
        });
      } catch (error) {
        console.warn('Navigation badges are unavailable:', error);
      }
    }
