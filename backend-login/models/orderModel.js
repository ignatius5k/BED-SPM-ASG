...
async function createOrder(customerId, stallId, items) {
  ...
  try {
    ...
    // Look up real prices server-side - never trust price from the client
    let total = 0;
    const priced = [];
    for (const item of items) {
      ...
      const priceResult = await priceReq.query(`
        SELECT Price FROM MenuItems WHERE MenuItemID = @menuItemId AND StallID = @stallId
      `);
...
async function getOrderById(orderId) {
  ...
  try {
    ...
    const orderResult = await orderReq.query(`
      SELECT o.*, s.StallName FROM Orders o
      JOIN Stalls s ON o.StallID = s.StallID
